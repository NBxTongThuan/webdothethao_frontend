...
const UpdateDiscountingPrice: React.FC<ModalProps> = (props) => {

    const [showConfirmModal, setShowConfirmModal] = useState(false);
    const [discountingPrice, setDiscountingPrice] = useState(props.product?.moneyOff);
    const handleDiscountingPriceChange = (e: React.ChangeEvent<HTMLInputElement>) => {
        ...
        if (Number(e.target.value) > props.product?.price) {
        ...
        if (Number(e.target.value) % 1000 !== 0) {
...
export default UpdateDiscountingPrice;
